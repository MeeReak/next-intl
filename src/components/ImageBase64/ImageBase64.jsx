  ...
  const handleFiles = (selected) => {
    Array.from(selected).forEach((file) => {
      ...
      if (fileRecord.tooBig || !isImage) {
        animateProgress(fileRecord.id, `${t("error")}`, true);
        return;
      }
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-gray-300 dark:border-gray-600">
                ...
                <th className="p-2">{t("fileSize")}</th>
                <th className="p-2">{t("progress")}</th>
                <th className="p-2">{t("encode")}</th>
          ...
          f.progress < 100
            ...
            : f.isError || f.tooBig
              ...
              : "bg-green-500" // success after done
                      ...
                      />
                      <span className="absolute inset-0 flex items-center justify-center dark:text-white text-black font-medium text-xs">
                        {f.progress < 100
                          ? `${t("progressing")}`
                          : f.isError || f.tooBig
                            ? `${t("error")}`
                              ...
                              <div className="flex gap-2">
                                ...
                                <button
                                  onClick={() => {
                                    ...
                                    updateFile(f.id, { copiedCsss: true });
                                    setTimeout(
                                      () =>
                                        updateFile(f.id, { copiedCsss: false }),
                                      1500
                                    );
                                  }}
                                  disabled={!f.base64}
